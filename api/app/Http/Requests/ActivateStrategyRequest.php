...
/**
 * Validates the HTTP shape of an "Aplicar" request. Mirrors
 * {@see SearchStrategiesRequest}'s trial/real handling: `mode=real` is
 * rejected outright, and `mode=trial` capital must not exceed the Binance
 * Demo account's available USDT balance.
 */
class ActivateStrategyRequest extends FormRequest
{
    /**
    ...
    {
        return [
            'strategy_name' => ['required', 'string', Rule::exists('strategies', 'name')],
    ...
    {
        return $this->string('strategy_name')->toString();
    }
