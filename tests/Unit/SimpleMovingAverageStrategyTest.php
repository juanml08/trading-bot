...
class SimpleMovingAverageStrategyTest extends TestCase
{
    public function test_returns_hold_when_there_are_not_enough_candles(): void
    {
        $signal = (new SimpleMovingAverageStrategy)->generate($this->candlesWithCloses(array_fill(0, 10, '100')));
    ...
    {
        $closes = [...array_fill(0, 10, '100'), '130'];
    ...
    {
        $signal = (new SimpleMovingAverageStrategy)->generate($this->candlesWithCloses(array_fill(0, 11, '100')));
